...
// var localStream = navigator.mediaDevices
//   .getUserMedia({
//     video: true,
//     audio: true,
//   })
  ...
  webSocket.onmessage = function (e) {
    ...
    if (send_type == "icecandidate_offer") {
      ...
      newSocketAnswer(
        jsonDic["sdp"],
        peer_data,
        jsonDic["receiver_peer"]["channel_name"]
      );
      return;
    }
    if (send_type == "icecandidate_answer") {
      ...
      peer.setRemoteDescription(answer).then(console.log("work done"));
      return;
    }
    if (send_type == "") {
...
function calendar_onclickEvent() {
  ...
  var days = document
    ...
    .forEach((item) => item.addEventListener("click", popSchedule()));
  console.log(days);
}
main();
